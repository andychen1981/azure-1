...
# wav0 include data/new_data and .wav eg. "data/new_data/20191101101519-003004802802-UP-00000003.wav"
# @app.route('/run_wav', methods = ['POST', 'GET'])
def run_wav(wav0):
    ...
    # c.execute("Update data_table (mea_date,dev_id,qr_code,loc,pulse,fft,trend) VALUES (?,?,?,?,?,?,?)",(mea_date,dev_id,qr_code,loc,wav1,wav1,wav1) where (last_name='West'))
	# c.execute("UPDATE data_table set mea_date=?,disp_date=?,dev_id=?,qr_code=?,loc=?,pulse=?,fft=?,trend=?,res=?,prob=? WHERE dev_id=?", (mea_date,disp_date,dev_id,qr_code,loc,wav1,wav1,wav1,res,conf,dev_id,) )
	c.execute("INSERT INTO data_table (first_name,last_name,mea_date,disp_date,dev_id,qr_code,loc,pulse,fft,trend,res,prob) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (first_name,last_name,mea_date,disp_date,dev_id,qr_code,loc,wav1,wav1,wav1,res,conf) )
